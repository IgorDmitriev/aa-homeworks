...
def top_titles
  ...
  Movie
    .select(:id, :title)
    .where('score >= :min_score', min_score: 9)
end
...
def star_wars
  ...
  Movie
    .select(:id, :title, :yr)
    .where('title LIKE :string_match', string_match: '%Star Wars%')
end
...
def uma_movies
  ...
  searching_name = 'Uma Thurman'

  p Movie
    .select('movies.id, title, yr')
    .joins(:super_actors)
    .where(actors: { name: searching_name }, movies: { yr: 1999 })
